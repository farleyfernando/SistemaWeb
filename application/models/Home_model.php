...
{
    public function get_soma_vendas(){

        $this->db->select([
            'FORMAT(SUM(REPLACE(venda_valor_total, ",", "")), 2) as venda_total',
        ]);
    ...
    public function get_produtos_mais_vendidos() {
        ...
        $this->db->select([
            'venda_produtos.*',
            'SUM(venda_produto_quantidade) as qtde_vendidos',    
            'produtos.produto_id',
        ...
        $this->db->order_by('qtde_vendidos', 'DESC');
    ...
    public function get_servicos_mais_vendidos() {
        ...
        $this->db->select([
            'ordem_tem_servicos.*',
            'SUM(ordem_ts_quantidade) as qtde_serv_vendidos',    
            'servicos.servico_id',
        ...
        $this->db->order_by('qtde_serv_vendidos', 'DESC');
